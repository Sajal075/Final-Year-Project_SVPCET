...
class BlockchainQRSystem:
    def __init__(self):
        ...
        # Electric Storm color theme
        self.colors = {
            'bg_primary': "#0d1117",      # GitHub dark
            'bg_secondary': "#161b22",    # Darker gray
            'accent_1': "#f0f6ff",        # Electric blue
            'accent_2': "#ffeb3b",        # Electric yellow
            'text_primary': "#ffffff",    # White
            'text_secondary': "#58a6ff",  # Light blue
    ...
    def show_login_page(self):
        ...
        title_label = tk.Label(login_frame, text="🔐 Blockchain QR System", 
                              font=("Arial", 24, "bold"), fg=self.colors['accent_1'], bg=self.colors['bg_secondary'])
        title_label.pack(pady=30, padx=50)
        
        # Subtitle
        subtitle_label = tk.Label(login_frame, text="Secure Product Authentication", 
                                 font=("Arial", 12), fg=self.colors['text_secondary'], bg=self.colors['bg_secondary'])
        ...
        self.username_entry = tk.Entry(login_frame, font=("Arial", 12), width=25, 
                                      bg="#21262d", fg=self.colors['text_primary'], relief="flat", bd=5)
        ...
        self.password_entry = tk.Entry(login_frame, font=("Arial", 12), width=25, 
                                      show="*", bg="#21262d", fg=self.colors['text_primary'], relief="flat", bd=5)
        self.password_entry.pack(pady=(0, 20))
        
        # Login button
        login_btn = tk.Button(login_frame, text="LOGIN", font=("Arial", 12, "bold"), 
                             bg=self.colors['accent_1'], fg=self.colors['bg_primary'], width=20, height=2,
        ...
        tk.Label(info_frame, text="Demo Credentials:", font=("Arial", 10, "bold"), 
                fg=self.colors['accent_2'], bg=self.colors['bg_secondary']).pack()
        tk.Label(info_frame, text="admin/admin123 | user/password | manufacturer/mfg2024", 
                font=("Arial", 9), fg=self.colors['text_secondary'], bg=self.colors['bg_secondary']).pack()
    ...
    def show_main_page(self):
        ...
        tk.Label(header_frame, text="🔗 Blockchain QR Verification System", 
                font=("Arial", 20, "bold"), fg=self.colors['accent_1'], bg=self.colors['bg_secondary']).pack(side="left", pady=20, padx=20)
        ...
        tk.Label(user_frame, text=f"Welcome, {self.current_user}", 
                font=("Arial", 12), fg=self.colors['text_primary'], bg=self.colors['bg_secondary']).pack(side="left", padx=(0, 10))
        
        logout_btn = tk.Button(user_frame, text="Logout", font=("Arial", 10), 
                              bg=self.colors['error'], fg=self.colors['text_primary'], relief="flat", 
        ...
        tk.Label(stats_frame, text="📊 System Statistics", font=("Arial", 14, "bold"), 
                fg=self.colors['accent_1'], bg=self.colors['bg_secondary']).pack(pady=10)
        ...
        tk.Label(stats_info, text=f"Total Registered Products: {total_products}", 
                font=("Arial", 12), fg=self.colors['text_primary'], bg=self.colors['bg_secondary']).pack(side="left", padx=20)
        
        tk.Label(stats_info, text=f"Current User: {self.current_user}", 
                font=("Arial", 12), fg=self.colors['text_primary'], bg=self.colors['bg_secondary']).pack(side="right", padx=20)
        ...
        # Generate QR Button
        generate_frame = tk.Frame(buttons_frame, bg=self.colors['bg_secondary'], relief="raised", bd=2)
        ...
        tk.Label(generate_frame, text="📱 Generate QR Code", font=("Arial", 16, "bold"), 
                fg=self.colors['accent_1'], bg=self.colors['bg_secondary']).pack(pady=20)
        
        tk.Label(generate_frame, text="Create encrypted QR codes for products\nand register them in the blockchain", 
                font=("Arial", 11), fg=self.colors['text_secondary'], bg=self.colors['bg_secondary'], justify="center").pack(pady=10)
        
        generate_btn = tk.Button(generate_frame, text="GENERATE QR", font=("Arial", 14, "bold"), 
                                bg=self.colors['accent_1'], fg=self.colors['bg_primary'], width=20, height=3,
                                relief="flat", cursor="hand2", command=self.show_generate_qr)
        generate_btn.pack(pady=20)
        
        # Verify QR Button
        verify_frame = tk.Frame(buttons_frame, bg=self.colors['bg_secondary'], relief="raised", bd=2)
        ...
        tk.Label(verify_frame, text="🔍 Verify QR Code", font=("Arial", 16, "bold"), 
                fg=self.colors['accent_2'], bg=self.colors['bg_secondary']).pack(pady=20)
        ...
        verify_btn = tk.Button(verify_frame, text="VERIFY QR", font=("Arial", 14, "bold"), 
                              bg=self.colors['accent_2'], fg=self.colors['text_primary'], width=20, height=3,
        ...
        try:
            ...
            for entry in reversed(recent_entries):
                entry_frame = tk.Frame(activity_frame, bg="#21262d")
                ...
                tk.Label(entry_frame, text=f"Product: {entry['product_id']} | Manufacturer: {entry['manufacturer']}", 
                        font=("Arial", 10), fg=self.colors['text_primary'], bg="#21262d").pack(side="left", pady=5, padx=10)
        except:
            tk.Label(activity_frame, text="No recent activity", 
                    font=("Arial", 10), fg=self.colors['text_secondary'], bg=self.colors['bg_secondary']).pack(pady=10)
    ...
    def show_generate_qr(self):
        ...
        back_btn = tk.Button(header_frame, text="← Back", font=("Arial", 12), 
                            bg="#30363d", fg=self.colors['text_primary'], relief="flat", 
        ...
        tk.Label(form_frame, text="Enter Product Information", font=("Arial", 16, "bold"), 
                fg=self.colors['text_primary'], bg=self.colors['bg_secondary']).pack(pady=20)
        ...
        generate_btn = tk.Button(form_frame, text="GENERATE QR CODE", font=("Arial", 14, "bold"), 
                                bg=self.colors['accent_1'], fg=self.colors['bg_primary'], width=25, height=2,
    ...
    def display_qr_code(self):
        ...
        try:
            ...
            tk.Label(self.qr_display_frame, text="QR Code Generated Successfully!", 
                    font=("Arial", 12, "bold"), fg=self.colors['success'], bg=self.colors['bg_secondary']).pack()
            
        except Exception as e:
            tk.Label(self.qr_display_frame, text=f"Error displaying QR: {str(e)}", 
                    font=("Arial", 10), fg=self.colors['error'], bg=self.colors['bg_secondary']).pack()
    ...
    def show_verify_qr(self):
        ...
        tk.Label(verify_frame, text="QR Code Verification", font=("Arial", 16, "bold"), 
                fg=self.colors['text_primary'], bg=self.colors['bg_secondary']).pack(pady=30)
        
        tk.Label(verify_frame, text="Click the button below to scan and verify the QR code", 
                font=("Arial", 12), fg=self.colors['text_secondary'], bg=self.colors['bg_secondary']).pack(pady=10)
    ...
    def verify_qr_code(self):
        ...
        try:
            ...
            scanning_label = tk.Label(self.result_frame, text="🔍 Scanning QR Code...", 
                                    font=("Arial", 14), fg=self.colors['accent_2'], bg=self.colors['bg_secondary'])
    ...
    def show_verification_result(self, is_authentic, product_info):
        ...
        if is_authentic:
            ...
            tk.Label(result_frame, text="✅ AUTHENTIC PRODUCT", font=("Arial", 18, "bold"), 
                    fg=self.colors['text_primary'], bg=self.colors['success']).pack(pady=15)
            
            info_frame = tk.Frame(result_frame, bg=self.colors['success'])
            info_frame.pack(pady=10)
            
            tk.Label(info_frame, text=f"Product ID: {product_info['product_id']}", 
                    font=("Arial", 12), fg=self.colors['text_primary'], bg=self.colors['success']).pack()
            ...
            tk.Label(result_frame, text="This product is verified and registered in the blockchain.", 
                    font=("Arial", 11), fg="#a3e635", bg=self.colors['success']).pack(pady=10)
        else:
            # Fake product
            ...
            tk.Label(result_frame, text="❌ FAKE PRODUCT DETECTED", font=("Arial", 18, "bold"), 
                    fg=self.colors['text_primary'], bg=self.colors['error']).pack(pady=15)
            
            tk.Label(result_frame, text="This product is NOT verified in the blockchain!", 
                    font=("Arial", 12, "bold"), fg=self.colors['text_primary'], bg=self.colors['error']).pack(pady=10)
            
            tk.Label(result_frame, text="⚠️ WARNING: This may be a counterfeit product", 
                    font=("Arial", 11), fg="#fecaca", bg=self.colors['error']).pack(pady=5)
