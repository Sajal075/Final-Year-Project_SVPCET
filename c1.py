...
class BlockchainQRSystem:
        ...
        try:
            ...
            for record in blockchain:
                if (record["product_id"] == product_id and
                    record["manufacturer"] == manufacturer and
                    record["batch_no"] == batch_no):
                    messagebox.showwarning("Already Registered", "This product is already registered in the blockchain!")
                    return
    ...
    def generate_qr_code(self):
        ...
        try:
            ...
            # Check if product is already registered in blockchain
            is_registered = self._verify_product(product_info)
            
            self._display_qr_code()
            
            if is_registered:
                messagebox.showinfo("Success", "QR Code generated successfully!.")
                self.register_product_button.pack_forget()  # Hide register button since already registered
            else:
                messagebox.showinfo("Success", "QR Code generated successfully!")
                self.register_product_button.pack(pady=(10, 0))  # Show register button
        except Exception as e:
