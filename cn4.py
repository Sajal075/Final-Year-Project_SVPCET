...
class BlockchainQRSystem:
    ...
    def _create_entry(self, parent, width, show=None, **kwargs):
        ...
        return tk.Entry(parent, font=("Arial", 12), width=width,
                         show=show, bg=self._get_color('activity_bg', '#334155'),
    ...
    def show_main_page(self):
        ...
        # Action buttons setup (now with a new "Register Product" button)
        button_configs = [
            ("📝 Register New Product", "Manually add a new product's details\ndirectly to the blockchain", "REGISTER", self.show_register_product, 'accent_2', "top"), # New button
            ("📱 Generate QR Code", "Create encrypted QR codes for products\n(Not yet registered in blockchain)", "GENERATE QR", self.show_generate_qr, 'accent_1', "left"),
        ...
        # Use a grid layout for better arrangement of 3 buttons
        # The new button will be at the top, others below
        top_button_frame = tk.Frame(buttons_frame, bg=self.colors['bg_primary'])
        top_button_frame.pack(pady=(0, 20)) # Padding below the top button

        # Register New Product button
        reg_frame = tk.Frame(top_button_frame, bg=self.colors['bg_secondary'], relief="raised", bd=2)
        reg_frame.pack(fill="both", expand=True, padx=20)
        self._create_label(reg_frame, button_configs[0][0], 16, "bold", color_key=button_configs[0][4], bg_key='bg_secondary').pack(pady=20)
        self._create_label(reg_frame, button_configs[0][1], 11, bg_key='bg_secondary', color_key='text_secondary', justify="center").pack(pady=10)
        ...
        bottom_buttons_frame = tk.Frame(buttons_frame, bg=self.colors['bg_primary'])
        bottom_buttons_frame.pack(fill="x", expand=True)

        # Generate QR and Verify QR buttons
        for i in range(1, len(button_configs)):
            config = button_configs[i]
            frame = tk.Frame(bottom_buttons_frame, bg=self.colors['bg_secondary'], relief="raised", bd=2)
            frame.pack(side="left" if config[5] == "left" else "right", padx=20, pady=20, fill="both", expand=True)
            self._create_label(frame, config[0], 16, "bold", color_key=config[4], bg_key='bg_secondary').pack(pady=20)
            self._create_label(frame, config[1], 11, bg_key='bg_secondary', color_key='text_secondary', justify="center").pack(pady=10)
            btn_fg_key = 'bg_primary' if config[4] == 'accent_1' else 'text_primary'
            self._create_button(frame, config[2], config[3], 14, width=20, height=3, bg_key=config[4], fg_key=btn_fg_key).pack(pady=20)
    ...
    def show_register_product(self):
        """Show interface for registering a new product directly to blockchain."""
        self.clear_frame()
        ...
        self._create_label(header_frame, "📝 Register New Product", 18, "bold", color_key='accent_2', bg_key='bg_secondary').pack(pady=15)
        ...
        self._create_label(form_frame, "Enter Product Details to Register", 16, "bold", bg_key='bg_secondary').pack(pady=20)

        fields = [
            ("Product ID:", "reg_product_id_entry"),
            ("Manufacturer:", "reg_manufacturer_entry"),
            ("Batch Number:", "reg_batch_no_entry")
        ]
    ...
    def register_new_product(self):
        """Registers a product directly based on input fields."""
        ...
        try:
            ...
            f = Fernet(key)
            encrypted_data = f.encrypt(json.dumps(product_info).encode()) # Even if not generating QR, data is "QR-ready" encrypted
            ...
            self.show_main_page() # Return to main page
    ...
    def display_qr_code(self):
        ...
        try:
            ...
            self._create_label(self.qr_display_frame, "Click 'REGISTER GENERATED PRODUCT' to add to blockchain.", 10, color_key='text_secondary', bg_key='bg_secondary').pack()
        except Exception as e:
